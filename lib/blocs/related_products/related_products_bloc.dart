...
class RelatedProductsBloc
  ...
  Stream<RelatedProductsState> mapEventToState(RelatedProductsEvent event) async* {
    if (event is GetRelatedProducts) yield await _mapProductsFetchedToState(state, event.categoryId);
  }

  Future<RelatedProductsState> _mapProductsFetchedToState(RelatedProductsState state, int? categoryId) async {
  ...
  Future<List<WooProduct>?> _fetchProducts([int? pageNumber = 1, int? categoryId]) async {
    final response = await productsRepo.fetchProductsByCat(pageNumber, categoryId, "date", "desc", "", "", "");
