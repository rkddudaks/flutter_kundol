...
class ProductsByCatBloc extends Bloc<ProductsByCatEvent, ProductsByCatState> {
  ...
  Future<ProductsByCatState> _mapFiltersChangedToState(
    ...
    try {
      ...
      attributeSlug = event.attributeSlug;
  ...
  Future<List<WooProduct>?> _fetchProducts([int? pageNumber = 1]) async {
    final response = await productsRepo.fetchProductsByCat(pageNumber,
        categoryId, sortBy, sortType, filters, attributeSlug, variationId);
