...
class CheckoutBloc extends Bloc<CheckoutEvent, CheckoutState> {
  ...
  Stream<CheckoutState> mapEventToState(CheckoutEvent event) async* {
    ...
    if (event is SaveOrder) {
      try {
        final saveOrderResponse =
            await checkoutRepo.saveOrder(event.orderDetails);
        if (saveOrderResponse != -1)
          yield CheckoutLoaded(saveOrderResponse);
        else
          yield CheckoutError("Some error occurred");
      } on Error {
        yield CheckoutError("Couldn't fetch weather. Is the device online?");
      }
    }
