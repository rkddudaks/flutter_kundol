...
class TopSellingProductsBloc
    extends Bloc<TopSellingProductsEvent, TopSellingProductsState> {
  ...
  @override
  Stream<TopSellingProductsState> mapEventToState(
      TopSellingProductsEvent event) async* {
    if (event is GetTopSellingProducts)
      yield await _mapProductsFetchedToState(state);
  }

  Future<TopSellingProductsState> _mapProductsFetchedToState(
      TopSellingProductsState state) async {
  ...
  Future<List<WooProduct>?> _fetchProducts([int? pageNumber = 1]) async {
